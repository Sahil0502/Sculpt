...
**Sculpt** is a performance intelligence platform built during a hackathon to enhance **manager ↔ employee 1-on-1 meetings**.
...
- Auto speaker identification using voice embeddings  
...
### 🤖 AI Insights Engine (RAG + GPT)
- Meeting summaries  
- Action items with priorities & deadlines  
- Feedback with competency mapping  
- Blind spot detection (what wasn’t discussed)  
- Growth insights across meetings  
...
- Detects stress, confidence, and engagement  
- Identifies ignored competencies  
- Tracks conversation balance  
- Generates a Meeting Health Score  
...
1. Store meeting data → chunk → embed  
2. Retrieve relevant history (top-K results)  
3. Inject into GPT prompt  
4. Generate:
   - Summary  
   - Feedback  
   - Action items  
...
- Multi-user meetings  
- Advanced speaker diarization (ML-based)  
- Emotion detection (voice + text fusion)  
...
This project was built during a hackathon, so some features are optimized for demo purposes rather than production scale.
